...
namespace TestHBTK
{
	...
	public:
		...
		TEST_METHOD(Static_integrator)
		{
			...
			Assert::AreEqual(20.0 / 3.0, result, num_points*HBTK::tolerance<double>(), L"Expected solution...", LINE_INFO());
		}
		...
		TEST_METHOD(Adaptive_simpsons_rule)
		{
			...
			auto my_f2 = [](double x)->double
			{return sin(x); };
			auto result2 = HBTK::adaptive_simpsons_integrate(my_f2, 1e-8, 0.0, 2.0);
			Assert::AreEqual(1.416146837, result2, 1e-8);
		}
